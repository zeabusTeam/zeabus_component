...
#ifndef _ZEABUS_CONTROL_FUZZY_ROLL_PARAMETER_HPP
#define _ZEABUS_CONTROL_FUZZY_ROLL_PARAMETER_HPP
...
namespace roll_parameter
{
    ...
    // Data is vector type
    const static std::array< double , 3 > ERROR_RULE = { 0.2 , 1 , 2 };

    // Diff is range to decision about velocity error will be 0 1 2 3 by use 3 value
    // Data is vertor type
    const static std::array< double , 3 > DIFF_RULE = { 0.05 , 0.30 , 0.50 };
    ...
    //  Data is vector type
    const static std::array< double , 4 > FORCE_RULE = { 0.1 , 1 , 2 , 4 };
    ...
    const static std::array< double , 3 > DEFUZZY_RULE = {0.05 , 0.2 , 0.4 };
